            ...
            <li>
                <div class="row">
                    <div class="col-lg-9 col-sm-7 col-xs-6 width100">
                        <div class="text-size14 cor-gray51"><a class="cor-blue2f" href="/task/release/{{ $v['id'] }}">{{ $v['title'] }}</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;{{ ($v['status']==0)?'未发布':'未托管' }}</div>
                    ...
                    <div class="col-lg-3 col-sm-5 col-xs-6 text-right g-unreleasebtn width100">@if($v['status']==1)<a  href="/task/bounty/{{ $v['id'] }}" target="_blank">托管</a>@endif<a href="/task/release/{{ $v['id'] }}" target="_blank">编辑</a><a  href="/user/unreleasedTasksDelete/{{ $v['id'] }}">删除</a></div>
                </div>
            </li>
            @endforeach
        </ul>
