...
class UserController extends UserCenterController
{
    ...
    public function loginApi(Request $request){
        if($_SERVER['REMOTE_ADDR']=='123.56.234.223'||1){
            $data=$request->all();
            if($data['token']==md5('xinanli'.md5(date('Ymd',time()).'xinanli'))){
                if(!UserModel::checkPassword($data['name'], $data['password'])){
                    echo json_encode(['status'=>false]);
                }else{
                    $user=UserModel::where('name', $data['name'])
                        ->orWhere('email', $data['name'])->orWhere('mobile', $data['name'])->first();
                    $return=[
                        'status'=>true,
                        'name'=>$user->name,
                        'email'=>$user->email,
