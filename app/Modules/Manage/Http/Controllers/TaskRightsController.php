...
class TaskRightsController extends ManageController
{
    ...
    {
        $data = $request->except('_token');
		if(!empty($data['worker_bounty']) || !empty($data['owner_bounty'])){
			return redirect()->back()->with(['error'=>'雇主或威客分配金额不能为空']);
		}
        $rights = TaskRightsModel::where('id',$data['id'])->first();
        
		$taskPaySection=TaskPaySectionModel::select('price')->where('task_id',$rights['id'])->where('status',1)->where('section_status',2)->first();        
        ...
        $status = DB::transaction(function() use($data,$rights,$bounty,$worker_id,$owner_id,$task)
        {
			...
			$taskPaySectionAll=TaskPaySectionModel::select('price','section_status')->where('task_id',$rights['id'])->where('status',1)->where('section_status',0)->get();
			...
			if(!sizeof($taskPaySectionAll)){
				...
				foreach($taskPaySectionAll as $Vtpsa){
				  $data['owner_bounty']+=floor($taskPaySectionAll['price']*$config['rule']*0.01);
				}
			}
